// Package auth implements the `papermap auth` CLI subcommands: login,
// logout, and whoami. The login form is a one-shot huh.Form so it stays
// independent of the main Bubble Tea program.
package auth
...
// papermapHuhTheme builds a huh theme tinted with the Papermap palette so
// the CLI form does not look like a stock huh demo. Colors are duplicated
// from internal/theme rather than imported to keep this package free of
// the bubbletea-only theme struct.
func papermapHuhTheme() huh.Theme {
