...
- [ ] Wire `context.Context` cancellation through SSE/HTTP insight goroutines so quit / Clear / workspace switch tear down cleanly (today they use `context.Background()`).
- [ ] Carry session-expiry detection through `insightHTTPResultMsg` (currently dropped in `app.go`'s HTTP-only error path, so users can stay signed-in with stale creds).
- [ ] Extract `Model.Update` (cyclomatic 53) into per-message handlers (`handleStartup`, `handleWorkspacesLoaded`, `handleInsight*`, `handleSessionExpired`, `handleKeyPress`).
- [ ] `GenerateRequestID` collisions: replace `time.Now()` modulo trick with `crypto/rand` or UUID.
- [ ] `startInsightWithRetry` uses `time.Sleep` ignoring ctx — switch to `time.NewTimer` + `select`.
- [ ] `sessionExpiredFromError` falls back to substring matching because some `fmt.Errorf("%v", err)` breaks `errors.Is`/`Unwrap`. Find the offending `%v` and switch to `%w`.
- [ ] Centralise duplicated overlay-centering logic (`overlayQuitDialog` vs `overlayWorkspacePicker`).
- [ ] Decide on dead `InsightResponse`/`InsightRequest` fields: trim or comment as forward-compat.
- [ ] Move repeated hex colors (`#2ED8A3`, etc.) into `internal/theme`.
